...
@app.function(
    ...
    min_containers=1,  # Keep 1 instance warm to reduce cold starts
)
...
def generate_response_web(request: dict):
    """Web endpoint for generating responses"""
    messages = request.get("messages", [])
    character = request.get("character", "georgian-gentleman")
    
    if not messages:
        return {"error": "Missing messages"}, 400

    result = generate_response.remote(messages, character)
